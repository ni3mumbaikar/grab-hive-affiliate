...
class InstagramGraphPublisherClient(InstagramPublisher):
    ...
    and an access token with the 'instagram_content_publish' permission.
    """
    ...
    def login(self) -> bool:
        """Authenticate / verify access token with Meta Graph API."""
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                logger.warning("Instagram (Graph API): Direct account verification returned status %d: %s. "
                               "This is common if the token lacks the 'instagram_basic' permission. "
                               "We will proceed with publishing anyway.", resp.status_code, resp.text)
            
            # Set logged in to True since we have configured credentials
            self.is_logged_in = True
            return True
        except Exception as e:
            logger.warning("Instagram (Graph API): Exception during account verification: %s. "
                           "We will proceed with publishing anyway.", e)
            self.is_logged_in = True
            return True

    def publish(self, image_path: str, caption: str, image_url: Optional[str] = None) -> Optional[str]:
        """Publish post image with caption to Instagram feed using Official Graph API."""
        if self.simulate:
        ...
        # Determine valid public image URL
        target_url = None
        ...
        if not target_url:
            logger.error(
                "Instagram (Graph API): Publish aborted. Meta Graph API requires a publicly accessible HTTP/HTTPS image URL. "
                "Neither image_url (%s) nor image_path (%s) is a valid web URL.",
        ...
        try:
            ...
            if resp.status_code != 200:
                logger.error("Instagram (Graph API): Container creation failed. Status: %d, Response: %s", 
            ...
            if not container_id:
                logger.error("Instagram (Graph API): Container creation response did not contain 'id': %s", container_data)
                return None
            ...
            try:
                ...
                if status_resp.status_code == 200:
                    ...
                    if status_code == "FINISHED":
                        ...
                        logger.error("Instagram (Graph API): Container processing failed with status: %s (%s)", 
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                # Check if it failed due to rate limit/action blocked but might have succeeded
                response_json = {}
                try:
                ...
                logger.error("Instagram (Graph API): Media publish failed. Status: %d, Response: %s", 
                             resp.status_code, resp.text)
                return None
        except Exception as e:
